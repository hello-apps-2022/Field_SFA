<template>
  <div class="flex h-screen w-screen overflow-hidden bg-white">
    ...
    <!-- Sidebar -->
    <aside
      class="flex h-full shrink-0 flex-col border-r border-gray-100 bg-gray-50 transition-[width] duration-200"
      :class="collapsed ? 'w-[52px]' : 'w-[220px]'"
    >
      <!-- Brand -->
      <div class="flex h-[52px] shrink-0 items-center border-b border-gray-100 px-3">
        ...
        <Transition name="fade-left">
          <span v-if="!collapsed" class="ml-2.5 truncate text-sm font-semibold text-gray-900">
            Hema SFA
          </span>
        </Transition>
        <button
...
<style scoped>
.fade-left-enter-active, .fade-left-leave-active { transition: opacity 0.15s ease, transform 0.15s ease; }
.fade-left-enter-from, .fade-left-leave-to { opacity: 0; transform: translateX(-4px); }
</style>
