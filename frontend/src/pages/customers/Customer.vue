      ...
      <div v-show="activeTab==='orders'" class="p-5">
        <div class="mb-3 flex flex-wrap items-center justify-between gap-2">
          ...
          <div class="flex items-center gap-2">
            ...
            <Btn variant="solid" icon="plus" size="sm" @click="orderFormPanel?.openCreate()">New Order</Btn>
          </div>
        </div>
    ...
    <div v-if="viewOrderDoc" class="space-y-4">
      ...
      <div v-if="viewState === 'Draft' || viewState === 'Confirmed'" class="flex flex-wrap gap-2 border-t border-gray-100 pt-4">
        <template v-if="viewState === 'Draft'">
          ...
          <Btn variant="subtle" size="sm" icon="edit-2" @click="goToOrder(viewOrderDoc.name)">Edit</Btn>
          <Btn variant="ghost" size="sm" icon="trash-2" @click="deleteDraft(viewOrderDoc.name)">Delete</Btn>
        </template>
        <template v-else>
          <Btn v-if="(viewOrderDoc.custom_sfa_order_type || 'Booking') === 'Booking'" variant="subtle" size="sm" icon="edit-2" @click="goToOrder(viewOrderDoc.name)">Edit Items</Btn>
...
async function onOrderSaved(p) { await load(); if (p && p.reopen && p.name) await openOrder({ name: p.name }) }
